...
Map::Map(Galaxy galaxy) {
    // Initialize planets
    for(int i = 0; i < NUMBER_OF_PLANETS; i++) {
        int x = GRID_X_ORIGIN + (galaxy.getXCord(i) * GRID_WIDTH) + PLANET_TEXTURE_OFFSET_X;
        int y = GRID_Y_ORIGIN + (galaxy.getYCord(i) * GRID_HEIGHT) + PLANET_TEXTURE_OFFSET_Y;
